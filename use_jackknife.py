...
def main():
    ...
    for i in range(s.chain.GetEntries()):
        s.chain.GetEntry(i)
        cands_vec = []
        ...
        print f.format(cov[0][0], cov[0][1], cov[1][1],sigU2, s.chain.met_sig, s.chain.met_pt, s.chain.cov_xx, s.chain.cov_xy, s.chain.cov_yy)
    ...
    #    print f.format(len(cands), 1, 1, 1, 1, max(cands), sum(cands), n_vert, math.sqrt(sum(cands)), 1, 1, math.sqrt(cov[0][0]), math.sqrt(cov[1][1]), met, s.chain.met_sumpt, njet, sigU2, sigU2,sigU2)
